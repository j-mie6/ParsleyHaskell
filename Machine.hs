...
exec :: TExpQ (PreparedInput (Rep O) O O#) -> (Machine O a, DMap MVar (LetBinding O a), [IMVar]) -> QST s (Maybe a)
...
execLogEnter :: String -> Exec s o xs ks a -> Reader (Ctx s o a) (Γ s o xs ks a -> QST s (Maybe a))
execLogEnter name k =
  do asks $! \ctx γ ->
