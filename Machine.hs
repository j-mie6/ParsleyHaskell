...
import Control.Monad.State.Strict (get, gets, put, modify', runState, evalState, State, MonadState)
...
newtype LetBinding a x = LetBinding (AbstractedStack (Free3 M Void3) a x)
newtype AbstractedStack k a x = AbstractedStack (forall xs ks. k xs ((x ': xs) ': ks) a)
abstract :: k xs ((x ': xs) ': ks) a -> AbstractedStack k a x
abstract k = AbstractedStack (coerce k)
letBind :: Free3 M Void3 xs ((x ': xs) ': ks) a -> LetBinding a x
letBind = LetBinding . abstract

instance Show (LetBinding a x) where show (LetBinding (AbstractedStack m)) = show m
...
data M k xs ks a where
  ...
  Call      :: Maybe (AbstractedStack k a x) -> !(MVar x) -> !(k (x ': xs) ks a) -> M k xs ks a
  Jump      :: Maybe (AbstractedStack k a x) -> !(MVar x) -> M k xs ((x ': xs) ': ks) a
  Empt      :: M k xs ks a
...
newtype Exec s xs ks a = Exec (State (Ctx s a) (Γ s xs ks a -> QST s (Maybe a)))
run :: Exec s xs ks a -> Γ s xs ks a -> Ctx s a -> QST s (Maybe a)
run (Exec m) γ ctx = evalState m ctx γ

exec :: TExpQ String -> (Machine a, DMap MVar (LetBinding a), ΣVars) -> QST s (Maybe a)
exec input (Machine !m, _, vss) = [||
  do xs <- makeX
     ks <- makeK
     hs <- makeH
     cs <- makeC
...
execHalt :: State (Ctx s a) (Γ s '[a] ks a -> QST s (Maybe a))
execHalt = return $! \γ -> [|| return $! Just $! peekX ($$(xs γ)) ||]

execRet :: State (Ctx s a) (Γ s (x ': xs) ((x ': xs) ': ks) a -> QST s (Maybe a))
execRet = do restore' <- asks (restore . σs); return $! \γ -> [|| do $$restore'; $$(resume γ) ||]

execCall :: Maybe (AbstractedStack (Exec s) a x) -> MVar x -> Exec s (x ': xs) ks a
         -> State (Ctx s a) (Γ s xs ks a -> QST s (Maybe a))
execCall (Just (AbstractedStack m)) μ (Exec k) =
  do mk <- k
     asks $! \ctx γ@(Γ xs ks o hs cs d) -> 
       [|| do let !(I# o#) = $$o
              let !(I# d#) = $$d + 1
              $$(save (σs ctx))
              runConcrete (fix (\r -> 
                AbsExec (\(!xs) (!ks) o# (!hs) (!cs) d# ->
                  ...
                  ))) $$xs $$(suspend mk γ) o# $$hs $$cs d# ||]
execCall Nothing μ (Exec k) =
  do QAbsExec m <- asks ((DMap.! μ) . μs)
     save' <- asks (save . σs)
     mk <- k
...
execJump :: Maybe (AbstractedStack (Exec s) a x) -> MVar x 
         -> State (Ctx s a) (Γ s xs ((x ': xs) ': ks) a -> QST s (Maybe a))
execJump (Just (AbstractedStack m)) μ =
  asks $! \ctx γ@(Γ xs ks o hs cs d) -> 
    [|| let !(I# o#) = $$o
            !(I# d#) = $$d
...
execPush :: WQ x -> Exec s (x ': xs) ks a -> State (Ctx s a) (Γ s xs ks a -> QST s (Maybe a))
execPush x (Exec k) = fmap (\m γ -> m (γ {xs = [|| pushX $$(_code x) $$(xs γ) ||]})) k

execPop :: Exec s xs ks a -> State (Ctx s a) (Γ s (x ': xs) ks a -> QST s (Maybe a))
execPop (Exec k) = fmap (\m γ -> m (γ {xs = [|| popX_ $$(xs γ) ||]})) k

execLift2 :: WQ (x -> y -> z) -> Exec s (z ': xs) ks a -> State (Ctx s a) (Γ s (y ': x ': xs) ks a -> QST s (Maybe a))
execLift2 f (Exec k) = fmap (\m γ -> m (γ {xs = [|| let !(# y, xs' #) = popX $$(xs γ); !(# x, xs'' #) = popX xs' in pushX ($$(_code f) x y) xs'' ||]})) k

execSat :: WQ (Char -> Bool) -> Exec s (Char ': xs) ks a -> State (Ctx s a) (Γ s xs ks a -> QST s (Maybe a))
execSat p (Exec k) =
  do mk <- k
     asks $! \ctx γ -> (nextSafe (skipBounds ctx) (input ctx) (o γ) (_code p) (\o c -> mk (γ {xs = [|| pushX $$c $$(xs γ) ||], o = o})) (raiseΓ γ))

execEmpt :: State (Ctx s a) (Γ s xs ks a -> QST s (Maybe a))
execEmpt = return raiseΓ

execCommit :: Bool -> Exec s xs ks a -> State (Ctx s a) (Γ s xs ks a -> QST s (Maybe a))
execCommit exit (Exec k) = local (\ctx -> if exit then addConstCount (-1) ctx else ctx)
                                 (fmap (\m γ -> m (γ {hs = [|| popH_ $$(hs γ) ||], cs = [|| popC_ $$(cs γ) ||]})) k)

execHardFork :: Exec s xs ks a -> Exec s xs ks a -> Maybe (ΦDecl (Exec s) x xs ks a) -> State (Ctx s a) (Γ s xs ks a -> QST s (Maybe a))
...
execSoftFork :: Maybe Int -> Exec s xs ks a -> Exec s xs ks a -> Maybe (ΦDecl (Exec s) x xs ks a) -> State (Ctx s a) (Γ s xs ks a -> QST s (Maybe a))
...
execJoin :: ΦVar x -> State (Ctx s a) (Γ s (x ': xs) ks a -> QST s (Maybe a))
execJoin φ = fmap (\(QJoin k) γ -> [|| let !(I# o#) = $$(o γ) in $$k (peekX $$(xs γ)) o# ||]) (asks ((DMap.! φ) . φs))

execAttempt :: Maybe Int -> Exec s xs ks a -> State (Ctx s a) (Γ s xs ks a -> QST s (Maybe a))
execAttempt constantInput (Exec k) =
  do mk <- inputSizeCheck constantInput k
     let handler = [||\(_ :: O#) hs cs d'# -> let !(# o, cs' #) = popC cs in raise hs cs' o (I# d'#)||]
     return $! \γ -> setupHandlerΓ γ handler mk

execLook :: Exec s xs ks a -> State (Ctx s a) (Γ s xs ks a -> QST s (Maybe a))
execLook k =
  let handler = [||\o# hs cs d'# -> raise hs (popC_ cs) (I# o#) (I# d'#)||]
  in asks (\ctx γ -> setupHandlerΓ γ handler (\γ' -> run k γ' ctx))

execNegLook :: Exec s xs ks a -> Exec s xs ks a -> State (Ctx s a) (Γ s xs ks a -> QST s (Maybe a))
...
execRestore :: Exec s xs ks a -> State (Ctx s a) (Γ s xs ks a -> QST s (Maybe a))
execRestore (Exec k) =
  do mk <- k
     return $! \γ -> [||
       let !(# o, cs' #) = popC $$(cs γ)
...
execCase :: Exec s (x ': xs) ks a -> Exec s (y ': xs) ks a -> State (Ctx s a) (Γ s (Either x y ': xs) ks a -> QST s (Maybe a))
...
execChoices :: forall x xs ks a s. [WQ (x -> Bool)] -> [Exec s xs ks a] -> State (Ctx s a) (Γ s (x ': xs) ks a -> QST s (Maybe a))
execChoices fs ks = 
...
execChainIter :: ΣVar x -> MVar x -> State (Ctx s a) (Γ s ((x -> x) ': xs) ((x ': xs) ': ks) a -> QST s (Maybe a))
execChainIter u μ =
  do ctx <- ask
     let !(QSTRef σ) = (σm ctx) DMap.! u
     let QAbsExec l = (μs ctx) DMap.! μ
...
execChainInit :: WQ x -> ΣVar x -> Exec s xs ((x ': xs) ': ks) a -> MVar x -> Exec s (x ': xs) ks a
              -> State (Ctx s a) (Γ s (x ': xs) ks a -> QST s (Maybe a))
execChainInit deflt u l μ k =
  do (ctx :: Ctx s a) <- ask
     let !(QSTRef σ) = (σm ctx) DMap.! u
     return $! \γ@(Γ xs ks o _ _ d) ->
       let handler = [||\o# hs cs d'# ->
             let !(# c, cs' #) = popC cs
...
execLogEnter :: String -> Exec s xs ks a -> State (Ctx s a) (Γ s xs ks a -> QST s (Maybe a))
execLogEnter name k =
  do ctx <- ask
     return $! \γ ->
...
execLogExit :: String -> Exec s xs ks a -> State (Ctx s a) (Γ s xs ks a -> QST s (Maybe a))
execLogExit name k =
  do ctx <- ask
     return $! \γ -> [|| trace $$(preludeString name '<' γ (debugDown ctx) (color Green " Good")) $$(run k γ (debugDown ctx)) ||]
...
setupJoinPoint :: Maybe (ΦDecl (Exec s) x xs ks a) -> State (Ctx s a) (Γ s xs ks a -> QST s (Maybe a)) -> State (Ctx s a) (Γ s xs ks a -> QST s (Maybe a))
setupJoinPoint Nothing mx = mx
setupJoinPoint (Just (φ, k)) mx =
  do ctx <- ask
     return $! \γ -> [||
       let join x (o# :: O#) = $$(run k (γ {xs = [||pushX x $$(xs γ)||], o = [||I# o#||]}) ctx)
       in $$(evalState mx (ctx {φs = DMap.insert φ (QJoin [||join||]) (φs ctx)}) γ)
       ||]

inputSizeCheck :: Maybe Int -> State (Ctx s a) (Γ s xs ks a -> QST s (Maybe a)) -> State (Ctx s a) (Γ s xs ks a -> QST s (Maybe a))
...
instance {-# OVERLAPS #-} MonadReader (Ctx s a) (State (Ctx s a)) where
  ask = get
  local f m = 
    do s <- get
       put (f s)
