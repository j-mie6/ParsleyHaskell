...
coinsNeeded :: Fix4 (M q o) xs n r a -> Int
coinsNeeded = fst . getConst4 . cata4 (Const4 . alg)
  where
    ...
    alg :: M q o (Const4 (Int, Bool)) xs n r a -> (Int, Bool)
    alg Ret                                = (0, False)
    ...
    alg (ChainInit _ _ _ (Const4 k))       = (0, snd k)
    alg (Join _)                           = (0, False)
    ...
    alg (Handler (Parsec h))               = getConst4 h
    alg (Handler _)                        = (0, False)
    alg (MetaM (AddCoins n) (Const4 k))    = k
    alg (MetaM (FreeCoins n) (Const4 k))   = (n, snd k)
    alg (MetaM (RefundCoins n) (Const4 k)) = (max (fst k - n) 0, snd k) -- These were refunded, so deduct
    alg (MetaM (DrainCoins n) (Const4 k))  = (fst k, False)