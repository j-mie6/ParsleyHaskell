...
codeGen :: PositionOps o => Maybe (MVar x) -> Fix (Combinator t) x -> Set IΣVar -> IMVar -> IΣVar -> LetBinding o t a x
codeGen letBound p rs μ0 σ0 = trace ("GENERATING " ++ name ++ ": " ++ show p ++ "\nMACHINE: " ++ show (map ΣVar (elems rs)) ++ " => " ++ show m) $ makeLetBinding m rs
  where
    ...
    alg :: PositionOps o => Combinator t (Cofree (Combinator t) (CodeGen o t a)) x -> CodeGen o t a x
...
pattern (:<$>:) :: Core.Defunc (a -> b) -> k a -> Combinator t (Cofree (Combinator t) k) b
pattern f :<$>: p <- (_ :< Pure f) :<*>: (p :< _)
pattern (:$>:) :: Combinator t (Cofree (Combinator t) k) a -> Core.Defunc b -> Combinator t (Cofree (Combinator t) k) b
pattern p :$>: x <- (_ :< p) :*>: (_ :< Pure x)
pattern LiftA2 :: Core.Defunc (a -> b -> c) -> k a -> k b -> Combinator t (Cofree (Combinator t) k) c
pattern LiftA2 f p q <- (_ :< ((_ :< Pure f) :<*>: (p :< _))) :<*>: (q :< _)
pattern TryOrElse ::  k a -> k a -> Combinator t (Cofree (Combinator t) k) a
...
peephole :: PositionOps o => Combinator t (Cofree (Combinator t) (CodeGen o t a)) x -> Maybe (CodeGen o t a x)
...
direct :: PositionOps o => Combinator t (CodeGen o t a) x -> Fix4 (Instr o t) (x : xs) (Succ n) r a -> CodeGenStack (Fix4 (Instr o t) xs (Succ n) r a)
