...
eval = cata4 (Machine . alg)
  where
    alg :: (?ops :: InputOps o, Ops o) => Instr o (Machine s o) xs n a r -> MachineMonad s o xs n a r
    alg Ret                 = evalRet
...
evalRet :: ContOps o => MachineMonad s o (x : xs) n x r
evalRet = return $! retCont >>= resume

evalCall :: ContOps o => MVar x -> Machine s o (x : xs) (Succ n) a r -> MachineMonad s o xs (Succ n) a r
evalCall μ (Machine k) = liftM2 (\mk sub γ@Γ{..} -> callWithContinuation sub (suspend mk γ) input handlers) k (askSub μ)

evalJump :: ContOps o => MVar x -> MachineMonad s o '[] (Succ n) x r
evalJump μ = askSub μ <&> \sub Γ{..} -> callWithContinuation sub retCont input handlers

evalPush :: Defunc x -> Machine s o (x : xs) n a r -> MachineMonad s o xs n a r
evalPush x (Machine k) = k <&> \m γ -> m (γ {operands = Op (genDefunc x) (operands γ)})

evalPop :: Machine s o xs n a r -> MachineMonad s o (x : xs) n a r
evalPop (Machine k) = k <&> \m γ -> m (γ {operands = let Op _ xs = operands γ in xs})

evalLift2 :: Defunc (x -> y -> z) -> Machine s o (z : xs) n a r -> MachineMonad s o (y : x : xs) n a r
evalLift2 f (Machine k) = k <&> \m γ -> m (γ {operands = let Op y (Op x xs) = operands γ in Op (genDefunc2 f x y) xs})

evalSat :: (?ops :: InputOps o, PositionOps o, BoxOps o, HandlerOps o) => Defunc (Char -> Bool) -> Machine s o (Char : xs) (Succ n) a r -> MachineMonad s o xs (Succ n) a r
...
evalEmpt :: (BoxOps o, HandlerOps o) => MachineMonad s o xs (Succ n) a r
evalEmpt = return $! raise

evalCommit :: Machine s o xs n a r -> MachineMonad s o xs (Succ n) a r
evalCommit (Machine k) = k <&> \mk γ -> let VCons _ hs = handlers γ in mk (γ {handlers = hs})

evalCatch :: (BoxOps o, HandlerOps o) => Machine s o xs (Succ n) a r -> Machine s o (o : xs) n a r -> MachineMonad s o xs n a r
evalCatch (Machine k) (Machine h) = liftM2 (\mk mh γ -> setupHandler γ (buildHandler γ mh) mk) k h

evalTell :: Machine s o (o : xs) n a r -> MachineMonad s o xs n a r
evalTell (Machine k) = k <&> \mk γ -> mk (γ {operands = Op (input γ) (operands γ)})

evalSeek :: Machine s o xs n a r -> MachineMonad s o (o : xs) n a r
evalSeek (Machine k) = k <&> \mk γ -> let Op input xs = operands γ in mk (γ {operands = xs, input = input})

evalCase :: Machine s o (x : xs) n a r -> Machine s o (y : xs) n a r -> MachineMonad s o (Either x y : xs) n a r
...
evalChoices :: [Defunc (x -> Bool)] -> [Machine s o xs n a r] -> Machine s o xs n a r -> MachineMonad s o (x : xs) n a r
...
evalIter :: (RecBuilder o, ReturnOps o, HandlerOps o)
         => MVar Void -> Machine s o '[] One Void r -> Machine s o (o : xs) n a r
         -> MachineMonad s o xs n a r
evalIter μ l (Machine h) = liftM2 (\mh ctx γ -> buildIter ctx μ l (buildHandler γ mh) (input γ)) h ask

evalJoin :: ContOps o => ΦVar x -> MachineMonad s o (x : xs) n a r
evalJoin φ = askΦ φ <&> resume

evalMkJoin :: JoinBuilder o => ΦVar x -> Machine s o (x : xs) n a r -> Machine s o xs n a r -> MachineMonad s o xs n a r
evalMkJoin = setupJoinPoint

evalSwap :: Machine s o (x : y : xs) n a r -> MachineMonad s o (y : x : xs) n a r
evalSwap (Machine k) = k <&> \mk γ -> mk (γ {operands = let Op y (Op x xs) = operands γ in Op x (Op y xs)})

evalDup :: Machine s o (x : x : xs) n a r -> MachineMonad s o (x : xs) n a r
evalDup (Machine k) = k <&> \mk γ ->
  let Op x xs = operands γ
  in dup x $ \dupx -> mk (γ {operands = Op dupx (Op dupx xs)})

evalMake :: ΣVar x -> Access -> Machine s o xs n a r -> MachineMonad s o (x : xs) n a r
evalMake σ a k = asks $! \ctx γ ->
  let Op x xs = operands γ
  in newΣ σ a x (run k (γ {operands = xs})) ctx

evalGet :: ΣVar x -> Access -> Machine s o (x : xs) n a r -> MachineMonad s o xs n a r
evalGet σ a k = asks $! \ctx γ -> readΣ σ a (\x -> run k (γ {operands = Op x (operands γ)})) ctx

evalPut :: ΣVar x -> Access -> Machine s o xs n a r -> MachineMonad s o (x : xs) n a r
...
evalLogExit :: (?ops :: InputOps o, PositionOps o, LogOps o) => String -> Machine s o xs n a r -> MachineMonad s o xs n a r
evalLogExit name (Machine mk) =
  liftM2 (\k ctx γ -> [|| trace $$(preludeString name '<' γ (debugDown ctx) (color Green " Good")) $$(k γ) ||])
    (local debugDown mk)
    ask

evalMeta :: (?ops :: InputOps o, PositionOps o, BoxOps o, HandlerOps o) => MetaInstr n -> Machine s o xs n a r -> MachineMonad s o xs n a r
