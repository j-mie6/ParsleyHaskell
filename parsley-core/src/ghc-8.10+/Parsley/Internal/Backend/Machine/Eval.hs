...
readyMachine :: (?ops :: InputOps (Rep o), Ops o, Trace) => Fix3 (Instr o) xs n r -> Machine s o a xs n r
readyMachine = cata3 (Machine . alg)
  where
    alg :: (?ops :: InputOps (Rep o), Ops o) => Instr o (Machine s o a) xs n r -> MachineMonad s o a xs n r
    alg Ret                 = evalRet
...
evalRet :: MachineMonad s o a (x : xs) n x
evalRet = return $! retCont >>= resume

evalCall :: forall s o a x xs n r. MarshalOps o => MVar x -> Machine s o a (x : xs) (Succ n) r -> MachineMonad s o a xs (Succ n) r
evalCall μ (Machine k) = freshUnique $ \u -> liftM2 (callCC u) (askSub μ) k

evalJump :: forall s o a x n. MarshalOps o => MVar x -> MachineMonad s o a '[] (Succ n) x
...
evalSat :: forall s o a xs n r. (?ops :: InputOps (Rep o), PositionOps (Rep o), Trace) => CharPred -> Machine s o a (Char : xs) (Succ n) r -> MachineMonad s o a xs (Succ n) r
evalSat p k@(Machine k') = do
  ...
  where
    satFetch :: Machine s o a (Char : xs) (Succ n) r -> MachineMonad s o a xs (Succ n) r
    ...
    emitCheckAndFetch :: Int -> Machine s o a (Char : xs) (Succ n) r -> MachineMonad s o a xs (Succ n) r
...
evalEmpt :: MachineMonad s o a xs (Succ n) r
...
evalCatch :: (PositionOps (Rep o), HandlerOps o) => Machine s o a xs (Succ n) r -> Handler o (Machine s o a) (o : xs) n r -> MachineMonad s o a xs n r
...
evalIter :: (RecBuilder o, PositionOps (Rep o), HandlerOps o)
         => MVar Void -> Machine s o a '[] One Void -> Handler o (Machine s o a) (o : xs) n r
...
evalJoin :: ΦVar x -> MachineMonad s o a (x : xs) n r
...
evalLogEnter :: (?ops :: InputOps (Rep o), LogHandler o, HandlerOps o)
             => String -> Machine s o a xs (Succ (Succ n)) r -> MachineMonad s o a xs (Succ n) r
...
evalLogExit :: (?ops :: InputOps (Rep o), PositionOps (Rep o), LogOps (Rep o)) => String -> Machine s o a xs n r -> MachineMonad s o a xs n r
evalLogExit name (Machine mk) =
  liftM2 (\k ctx γ -> [|| Debug.Trace.trace $$(preludeString name '<' γ (debugDown ctx) (color Green " Good")) $$(k γ) ||])
    (local debugDown mk)
    ask

evalMeta :: (?ops :: InputOps (Rep o), PositionOps (Rep o)) => MetaInstr n -> Machine s o a xs n r -> MachineMonad s o a xs n r
