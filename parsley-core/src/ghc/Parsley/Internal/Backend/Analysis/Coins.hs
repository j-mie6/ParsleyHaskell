...
alg _     (Join _)                                = (zero, False) -- this is zero because a DrainCoins is generated just in front!
...
alg _     (Put _ _ (Const4 k))                    = first (const zero) k -- cannot allow factoring through a put, because it could stop it from executing
