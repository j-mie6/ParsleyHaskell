...
alg Fail{}              n         = replicateVec n False
alg Raise               n         = replicateVec n False
alg (RelabelError _ k)  (SSucc n) = VCons True (getStack k n)
alg (RelabelGhosts _ k) (SSucc n) = VCons True (getStack k n)
alg (Explain _ k)       (SSucc n) = VCons True (getStack k n)
alg (Amend k)           (SSucc n) = VCons True (getStack k n)
alg (Entrench k)        n         = getStack k n
