...
parsecHandler :: Opt.Flags -> Fix4 (Instr o) xs (Succ n) r a -> Handler o (Fix4 (Instr o)) (o : xs) (Succ n) r a
parsecHandler flags k = Same (not (shouldInline flags k)) k False (In4 Empt)

recoverHandler :: Opt.Flags -> Fix4 (Instr o) xs n r a -> Handler o (Fix4 (Instr o)) (o : xs) n r a
recoverHandler flags = Always . not . shouldInline flags <*> In4 . Seek

altCompile :: Trace => Opt.Flags -> CodeGen o a y -> CodeGen o a x
...
altCompile flags p q handler post m =
  do (binder, φ) <- makeΦ flags m
...
deep :: Trace => Opt.Flags -> Combinator (Cofree Combinator (CodeGen o a)) x -> Maybe (CodeGen o a x)
deep _     (f :<$>: (p :< _)) = Just $ CodeGen $ \m -> runCodeGen p (In4 (_Fmap (user f) m))
deep flags (TryOrElse p q) = Just $ CodeGen $ altCompile flags p q (recoverHandler flags) id
deep flags ((_ :< (Try (p :< _) :$>: x)) :<|>: (q :< _)) = Just $ CodeGen $ altCompile flags p q (recoverHandler flags) (In4 . Pop . In4 . Push (user x))
deep flags ((_ :< (f :<$>: (_ :< Try (p :< _)))) :<|>: (q :< _)) = Just $ CodeGen $ altCompile flags p q (recoverHandler flags) (In4 . _Fmap (user f))
deep _     _ = Nothing

addCoinsNeeded :: Fix4 (Instr o) xs (Succ n) r a -> Fix4 (Instr o) xs (Succ n) r a
addCoinsNeeded = coinsNeeded >>= addCoins

shallow :: Trace => Opt.Flags -> Combinator (CodeGen o a) x -> Fix4 (Instr o) (x : xs) (Succ n) r a -> CodeGenStack (Fix4 (Instr o) xs (Succ n) r a)
shallow _     (Pure x)      m = do return $! In4 (Push (user x) m)
shallow _     (Satisfy p)   m = do return $! In4 (Sat p m)
shallow _     (pf :<*>: px) m = do pxc <- runCodeGen px (In4 (_App m)); runCodeGen pf pxc
shallow _     (p :*>: q)    m = do qc <- runCodeGen q m; runCodeGen p (In4 (Pop qc))
shallow _     (p :<*: q)    m = do qc <- runCodeGen q (In4 (Pop m)); runCodeGen p qc
shallow _     Empty         _ = do return $! In4 Empt
shallow flags (p :<|>: q)   m = do altCompile flags p q (parsecHandler flags) id m
shallow _     (Try p)       m = do fmap (In4 . flip Catch rollbackHandler) (runCodeGen p (deadCommitOptimisation m))
shallow _     (LookAhead p) m =
...
shallow flags (NotFollowedBy p) m =
  do pc <- runCodeGen p (In4 (Pop (In4 (Seek (In4 (Commit (In4 Empt)))))))
     ...
     return $! In4 (Catch (blockCoins True (addCoinsNeeded (In4 (Tell pc)))) (Always (not (shouldInline flags m)) (In4 (Seek (In4 (Push (user UNIT) m))))))
shallow flags (Branch b p q) m =
  do (binder, φ) <- makeΦ flags m
...
shallow flags (Match p fs qs def) m =
  do (binder, φ) <- makeΦ flags m
...
shallow _     (Let _ μ)                    m = do return $! In4 (Call μ m)
shallow flags (Loop body exit)             m =
  do μ <- askM
     bodyc <- freshM (runCodeGen body (In4 (Pop (In4 (_Jump μ)))))
     exitc <- freshM (runCodeGen exit m)
     return $! In4 (Iter μ (addCoinsNeeded bodyc) (parsecHandler flags (addCoinsNeeded exitc)))
shallow _     (MakeRegister σ p q)         m = do qc <- runCodeGen q m; runCodeGen p (In4 (_Make σ qc))
shallow _     (GetRegister σ)              m = do return $! In4 (_Get σ m)
-- seems effective: blocks upstream coins from commuting down, but allows them to self factor
shallow _     (PutRegister σ p)            m = do runCodeGen p (In4 (_Put σ (In4 (Push (user UNIT) (blockCoins False m)))))
shallow _     (Position sel)               m = do return $! In4 (SelectPos sel m)
shallow _     (Debug name p)               m = do fmap (In4 . LogEnter name) (runCodeGen p (In4 (Commit (In4 (LogExit name m)))))
-- make sure to issue the fence after `p` is generated, to allow for a (safe) single character factor
shallow _     (MetaCombinator Cut p)       m = do runCodeGen p (blockCoins False (addCoinsNeeded m))
...
makeΦ :: Trace => Opt.Flags -> Fix4 (Instr o) (x ': xs) (Succ n) r a -> CodeGenStack (Fix4 (Instr o) xs (Succ n) r a -> Fix4 (Instr o) xs (Succ n) r a, Fix4 (Instr o) (x : xs) (Succ n) r a)
makeΦ flags m | shouldInline flags m = trace ("eliding " ++ show m) $ return (id, m)
  {-where
    ...
    elidable _                          = False-}
makeΦ _ m = let n = coinsNeeded m in fmap (\φ -> (In4 . MkJoin φ (giveBursary n m), drainCoins n (In4 (Join φ)))) askΦ
