...
import Parsley.Internal.Backend.Machine.Types       (Func, Regs(..))
...
letRec bindings nameOf genBinding wrapBinding expr = unsafeCodeCoerce $
  do -- Make a bunch of names
     names <- traverseWithKey (\k (LetBinding _ rs hs ys meta) -> Const . (, rs, hs, ys, meta) <$> newName (nameOf k)) bindings
  ...
  where
     makeTypedName :: Const (Name, Some Regs, Some Regs, Some Regs, Metadata) x -> binding s o a x
     makeTypedName (Const (name, Some frees, Some hFrees, Some rFrees, meta)) = wrapBinding (unsafeCodeCoerce (return (VarE name))) frees hFrees rFrees meta
