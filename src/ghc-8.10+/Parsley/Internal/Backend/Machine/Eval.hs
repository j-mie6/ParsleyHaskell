...
eval :: forall o a. (Trace, Ops o) => Code (InputDependant (Unboxed o)) -> (LetBinding o a a, DMap MVar (LetBinding o a)) -> Code (Maybe a)
...
readyMachine :: (?ops :: InputOps (Unboxed o), Ops o, Trace) => Fix4 (Instr o) xs n r a -> Machine s o xs n r a
readyMachine = cata4 (Machine . alg)
  where
    alg :: (?ops :: InputOps (Unboxed o), Ops o) => Instr o (Machine s o) xs n r a -> MachineMonad s o xs n r a
    alg Ret                 = evalRet
...
evalSat :: (?ops :: InputOps (Unboxed o), PositionOps o, Trace) => Defunc (Char -> Bool) -> Machine s o (Char : xs) (Succ n) r a -> MachineMonad s o xs (Succ n) r a
...
evalLogEnter :: (?ops :: InputOps (Unboxed o), LogHandler o) => String -> Machine s o xs (Succ (Succ n)) r a -> MachineMonad s o xs (Succ n) r a
...
evalLogExit :: (?ops :: InputOps (Unboxed o), PositionOps o, LogOps (Unboxed o)) => String -> Machine s o xs n r a -> MachineMonad s o xs n r a
evalLogExit name (Machine mk) =
  liftM2 (\k ctx γ -> [|| Debug.Trace.trace $$(preludeString name '<' γ (debugDown ctx) (color Green " Good")) $$(k γ) ||])
    (local debugDown mk)
    ask

evalMeta :: (?ops :: InputOps (Unboxed o), PositionOps o) => MetaInstr n -> Machine s o xs n r a -> MachineMonad s o xs n r a
